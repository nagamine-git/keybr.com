...
export function chordEmulation(
  ...
  const pendingPreModifiers: Map<
    KeyId,
    { timeStamp: number; event: IKeyboardEvent }
  > = new Map();
  ...
  return {
    onKeyDown: (event: IKeyboardEvent): void => {
      ...
      const hasModifiers =
        event.modifiers.includes("Shift") || event.modifiers.includes("Alt");
      if (isPreModifier(event.code) && !hasModifiers) {
        ...
        if (pendingPreModifiers.size > 0) {
          ...
          preModifierTimeout = setTimeout(
            clearPendingPreModifiers,
            PRE_MODIFIER_TIMEOUT,
          );
    ...
    onInput: (event: IInputEvent): void => {
      ...
      if (event.inputType !== "appendChar") {
        if (
          event.inputType === "clearChar" ||
          event.inputType === "clearWord"
        ) {
          // Clear both buffers
          postModBuffer.clear();
          clearPendingPreModifiers();
